...
/// FixedThreadPool holds threads.
class FixedThreadPool : public AbstractThreadPool {
public:
    explicit FixedThreadPool();  ///< Constructor.
    virtual ~FixedThreadPool();  ///< Destructor.
    ...
    /// Append a job to the pool, might be scheduled when we have an idle thread.
...
private:
    ...
    typedef std::vector<internal::Thread *> threads_t;
